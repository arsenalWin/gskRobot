...
public class TipsMotion
{
	...
	{
        get { return FuncPara.speedRate; }
        set { FuncPara.speedRate = value; }
	}
	//暂停状态
    public bool PauseControl
    {
        get { return !FuncPara.isPlaying; }
        set { FuncPara.isPlaying = !value; }
    }

	//Tips信息
	public string TipsString = "";
	//Tips信息
	public string TipsReadString = "";
	//Tips播放速度
	...
	public bool TimesUp(float current_time)
	{
		...
		//Debug.Log(current_time + "," + _pauseTimeRecorder + "," + EndTime);
...
public class TipsInfoManager
{
	...
	public TipsMotion TipsInfoGet(DataRow tips_row, string id, ref bool is_right)
	{
		...
		//多音字判断
		if(tips_row.Table.Columns.Count > 5)
		{
			_tipsMotion.TipsReadString = (string)tips_row[5].ToString().Trim();
			if(_tipsMotion.TipsReadString == "")
				_tipsMotion.TipsReadString = _tipsMotion.TipsString;
		}
		else
		{
			_tipsMotion.TipsReadString = _tipsMotion.TipsString;
		...
		//处理文本播放时长
		_tipsMotion.StandardTime = GetPlayTime(_tipsMotion.TipsReadString);
			...
			if(pos_str.ToLower() == "down_left"){
				...
				if(strArray.Length != 2){
					...
					try{
						...
						_tipsMotion.PosString = "down_left";
						_tipsMotion.IsString = true;
						Debug.LogWarning(", 位置填写错误，默认为down_left");
						is_right = false;
						return;
					}
		...
		if(bool_str == ""){
			...
			if(bool_str == "TRUE"){
				...
				if(grid_type.ToUpper() == "TITLE"){
					Debug.LogWarning(", 标题填写错误，默认为非标题");
					return false;
				}else if(grid_type.ToUpper() == "MOVE"){
					Debug.LogWarning(", 移动选项填写错误，默认为非移动");
					return false;
				}else{
					Debug.LogWarning(", 同步运动选项填写错误，默认为同步");
					return true;
				}
			}
		}
