...
public class JsonOperator
{
	...
	//获得file_path的Json数据文件中所有的数据表
	public DataSet JsonReader(string file_path)
	{
		string jsonText = @"";
		jsonText += File.ReadAllText(file_path);
		DataSet dataSet = JsonConvert.DeserializeObject<DataSet>(jsonText);
	...
	public bool JsonWriter(string file_path, string sheet_name, string[] column_name, string[,] insert_content, bool over_flag)
	{
		...
		if (File.Exists(file_path))
		{
			...
			//如果已包含
			if (dataSet.Tables.Contains(sheet_name))
			{
				dataSet.Tables.Remove(sheet_name);
			}
		}
		else
		{
	...
	//删除Json中的参数
	public bool JsonDelete(string file_path, string sheet_name)
	{
		...
		{
//			UnityEngine.Debug.LogError(file_path + "文件中不包含" + sheet_name);
		}
